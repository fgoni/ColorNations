...
function calcularInjureds($attacker, $defender){
	...
	//echo $ratio," ", $ar, " ", $dr, " ", "<br />";
	$injureds = array('attacker' => round(mt_rand($a_units/500, $a_units/100)*$ar), 'defender' => round(mt_rand($d_units/500, $d_units/100)*$dr));
...
function updateRank(){
	...
	while ($row = mysql_fetch_object($sql)){ //Los inserto desordenados, en pares de Valor-Usuario
		$attack[$i] = array(calcularAttack($row), $row->usuario);
		$defense[$i] = array(calcularDefense($row), $row->usuario);
		$i++;
	}
	...
	foreach ($attack as $key=>$value){//0=ataque, 1=usuario, 2=ranking
		$attack[$key] = array($value[0], $value[1], $key+1);
	}
	foreach ($defense as $key=>$value){//0=ataque, 1=usuario, 2=ranking
		$defense[$key] = array($value[0], $value[1], $key+1);
	}
	$sql = mysql_query("SELECT * FROM usuarios"); //Vuelvo a correr la consulta para el segundo while.
	$i = 0;
	while ($row = mysql_fetch_object($sql)){
		$overall[$i] = array(getRankF($row->usuario, $attack)+getRankF($row->usuario, $defense), $row->usuario, calcularAttack($row)+calcularDefense($row));
		$i++;
	}
	...
	foreach ($overall as $key=>$value){//0=sumaRankings, 1=sumaStats, 2=usuario, 3=ranking
		$overall[$key] = array($value[0], $value[1], $value[2], $key+1);
	}
